...
class QueryInfo:
    ...
    def get_info(self):
        ...
        try:
            ...
            conn = psycopg2.connect(host="localhost", database="template1", user="gpadmin")
            cur = conn.cursor()
            cur.execute('SELECT datname, usename, state, query from pg_stat_activity() where pid = %s' % self.pid)
...
class IOTopUI(object):
    ...
    sorting_keys = [
        ...
        (lambda p, s: p.username, True),
        (lambda p, s: p.dbname, True),
        (lambda p, s: s.start_time, True),
        (lambda p, s: s.submit_time, True),
        (lambda p, s: s.status, True),
        (lambda p, s: p.query_text, False),
    ]

    def __init__(self, win, queries_list, options):
    ...
    def run(self):
        ...
        while self.options.iterations is None or \
            ...
            total, actual = self.queries_list.refresh_processes()
            self.refresh_display(iterations == 0, total, actual,
                                 self.queries_list.duration)
    ...
    def handle_key(self, key):
        ...
        def toggle_processes():
            ...
            self.queries_list.refresh_processes()
        def ionice():
            try:
                if self.options.processes:
                    pid = self.prompt_pid()
                    exec_unit = self.queries_list.get_process(pid)
                else:
                    tid = self.prompt_tid()
                    exec_unit = ThreadInfo(tid,
                                         self.queries_list.taskstats_connection)
    ...
    def get_data(self):
        def format(p):
            stats = format_stats(self.options, p, self.queries_list.duration)
        ...
        processes = list(filter(should_format,
                                self.queries_list.processes.values()))
